...
DEFINE VARIABLE iNRJ AS INTEGER     NO-UNDO.

DO i = 1 TO 4:
    /* MESSAGE SUBSTITUTE("pos=<x=&1, y=&2, z=&3>, vel=<x=&4, y=&5, z=&6>", iX[i],iY[i],iZ[i],iVX[i],iVY[i],iVZ[i]) */
        /* VIEW-AS ALERT-BOX INFO BUTTONS OK. */
    iNRJ = iNRJ + (ABS(iX[i]) + ABS(iY[i]) + ABS(iZ[i])) * (ABS(iVX[i]) + ABS(iVY[i]) + ABS(iVZ[i])).
END.

MESSAGE ETIME SKIP iNRJ
    VIEW-AS ALERT-BOX INFO BUTTONS OK.
/*
---------------------------
